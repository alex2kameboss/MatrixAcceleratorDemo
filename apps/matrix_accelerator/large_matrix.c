...
void acc_mult(int32_t* a, int32_t* b, int32_t *c, int32_t m, int32_t n, int32_t p) {
    ...
    for ( int32_t i = 0; i < m; i += 512 )
        for ( int32_t j = 0; j < p; j += 512 ) {
            for ( int32_t k = 0 ; k < n; k+= 512 ) {
                ...
                MA_DEFINE_int32_t(1, 512, 512); 
                MA_LOC_RECT(1, 512, 0); // acc.define_matrix(1, 512, 512, 512, 0);
    ...
    printf("%llu,%llu,%llu,", mem_time, compute_time, mem_time + compute_time);
}
...
void sw_mult(int32_t* a, int32_t* b, int32_t *c, int32_t m, int32_t n, int32_t p) {
    clear_timer();
    start_timer();
    for (int i = 0; i < m; ++i) {
        for (int j = 0; j < p; ++j) {
    ...
    stop_timer();
    printf("%llu", get_timer_value());
}

int main() {
    printf("size,acc_mem_time,acc_compute_time,acc_total_time\n\r");
    for ( int i = 1; i <= 26; i++ ) {
        int n = i * 1024;
        //printf("Size: %d\r\n", n);
        printf("%d,", n);
